...
export async function getAllCourses() {
  const { data, error } = await supabase.from('courses').select('id, category_id, title, level, duration, thumbnail_url, is_free, price').eq('status', 'live');
  if (error) throw error;
  return (data || []).map((c) => ({
    id: c.id,
...
/* Batched "what's my status on each course" lookup for the catalog grid —
   one query for every enrollment instead of a per-card check. Keyed by
   course_id for O(1) card-level lookup. */
export async function fetchMyEnrollmentMap(userId) {
  if (!userId) return new Map();
  const { data, error } = await supabase
    .from('user_courses')
    .select('course_id, access_level, completed_at, progress_percentage')
    .eq('user_id', userId);
  if (error) throw error;
  const map = new Map();
  (data || []).forEach((r) => map.set(r.course_id, r));
  return map;
}
...
export async function getCoursesByIds(ids) {
  ...
  const { data, error } = await supabase.from('courses').select('id, title, level, duration, price').in('id', uniqueIds);
  if (error) throw error;
  const map = new Map();
