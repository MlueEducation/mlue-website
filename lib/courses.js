...
export async function getCourseById(id) {
  const { data: courseRow, error } = await supabase.from('courses').select('*').eq('id', id).maybeSingle();
...
export async function getAllCourses() {
  const { data, error } = await supabase.from('courses').select('id, category_id, title, level, duration');
  if (error) throw error;
  return (data || []).map((c) => ({
    id: c.id,
